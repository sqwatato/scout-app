...
import { ScrollView, Text, Alert, View, TouchableOpacity, BackHandler } from 'react-native';
import { auth, db } from '../firebase';
import { Button, IndexPath, Input, Select, SelectItem, Spinner, Toggle } from '@ui-kitten/components';
import { Modal, Card, Radio, RadioGroup, StyleService, useStyleSheet, ModalService, Layout } from '@ui-kitten/components';
...
const selectRegionalTheme = StyleService.create({
    container: {
        minHeight: 192,
...
const PitScoutForm: FC<PitScoutProps> = ({ navigation }) => {
    const styles = useStyleSheet(selectRegionalTheme);
    ...
    const [visibleRegional, setVisibleRegional] = useState<boolean>(false);
    ...
    return (!loading ?
        <View>
            ...
            >
                ...
                <Button onPress={() => setVisibleRegional(true)}>
                    Set Regional
                </Button>
                <Modal 
                    visible={visibleRegional}
                    // isBackDropAllowed 
                    backdropStyle={styles.backdrop}
                    onBackdropPress={() => setVisibleRegional(false)}
                >
                    <Card disabled={true}>
                        ...
                        <RadioGroup
                            ...
                            onChange={(index) => {
                                ...
                                setRegional(regionals[parseInt(index.toString())]);
                                // setRegionals(regionals[parseInt(index.toString())]);
                            }}
                        >
                            {regionals.map(r => <Radio>{evaProps => <Text {...evaProps}>{r}</Text>}</Radio>)}
                        </RadioGroup>
                    </Card>
                </Modal>
                <Select
